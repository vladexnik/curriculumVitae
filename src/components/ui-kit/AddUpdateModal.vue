...
const header = computed(() => {
  return type?.value === HEADER_TYPES.ADD
    ? `${HEADER_TYPES.ADD} ${name?.value}`
    : `${HEADER_TYPES.UPDATE} ${name?.value}`
  // switch (type?.value) {
  //   case HEADER_TYPES.ADD:
  //     return `${HEADER_TYPES.ADD} ${name?.value}`
  //   case HEADER_TYPES.UPDATE:
  //     return `${HEADER_TYPES.UPDATE} ${name?.value}`
  //   default:
  //     return
  // }
})
